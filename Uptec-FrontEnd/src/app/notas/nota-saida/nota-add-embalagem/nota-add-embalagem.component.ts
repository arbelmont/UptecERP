...
import { NotaSaidaAdd, NotaSaidaItemAdd, NotaSaidaItem } from '../models/nota-saida';
...
@Component({
  selector: 'app-nota-add-embalagem',
  templateUrl: './nota-add-embalagem.component.html'
})
export class NotaAddEmbalagemComponent extends BaseListComponet implements OnInit {
  ...
  getLotes() {
    this.notaService.getLoteEmbalagemToNfeSaida(this.destinatario.id, this.tipoDestinatario).pipe(take(1))
  ...
  private novaNota(): NotaSaidaAdd {
    return {
      ...
      valorDesconto: 0,
      outrasInformacoes: "",
      tipoNota: TipoNotaSaida.Embalagem,
      ordemItens: [],
      loteItens: []
    };
  }
