...
export class SidebarComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.menuItems = ROUTES.filter(menuItem => menuItem);
        ...
        for (let index = 1; index < this.menuItems.length; index++) {
            let el = this.menuItems[index];
            let remove = 0;

            el.role.forEach(rl => {
                var userRole = this.authService.getUser()['claims'].find(x => x.type.includes("role") && x.value === rl);
